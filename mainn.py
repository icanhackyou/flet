...
global cred, otp_, trys_, otp_gen_time, email_, login_status
login_status=None

def main(page: ft.Page):
    global current_page, page_banner, otp_, otp_gen_time, trys_
    otp_=0
    current_page=0
    ...
    def check_otp(i):
        global otp_, trys_, otp_gen_time, otp_login_singup, login_status, email_
        if otp_==0:
            pass
        else:
            otp=otp_login_singup.value
            current_time=datetime.now(timezone("Asia/Kolkata")).strftime("%Y-%m-%d %H:%M:%S:%f")
            value=datetime.strptime(current_time, "%Y-%m-%d %H:%M:%S:%f")-datetime.strptime(otp_gen_time, "%Y-%m-%d %H:%M:%S:%f")
            ...
            if len(otp)==6 or len(otp)>6 or i=="continue_user":# here has to put condition ==5 and >5 since the reponse is always 1 keyword buffer so to fullfill the requirement of 6 char otp simply lower it to 5 hence when user type 6 dig simply logic gets true
                if trys_<=6 and int(passed_minutes)<=4: #if have to set time for 5 minutes set condition : int(passed_minutes)<4
                    if i=="continue_user":
                        pass
                    else:
                        if int(otp)==otp_:
                            ...
                            trys_=0
                            otp_gen_time=0
                            login_status=email_ #hence if user logs out simply set login_status=none and use it to forward from account page to bookings page
                            register_load_user_data(email_)
                            ...
                            otp_login_singup.read_only=True # fast wpm attack protection vvi security feature
                            page.update()
                            otp_login_singup.value=""
                            otp_login_singup.read_only=False
                            trys_+=1
                            page.update()
                else:
                    otp_login_singup.value=""
                    otp_login_singup.label=""
                    otp_login_singup.hint_text="Sending New OTP"
                    otp_login_singup.read_only=True
    ...
    page.bgcolor="#F5F5F5"  
    page_banner=ft.Banner(content=ft.Text("There was an Error"),actions=[ft.TextButton("Dismiss", on_click=close_banner),],bgcolor=ft.colors.AMBER_100,leading=ft.Icon(ft.icons.WARNING_AMBER_ROUNDED, size=40))
    ...
    def send_sended_otp(choice):
        global otp_login_singup, page_banner, otp_, trys_, otp_gen_time, email_, email_login_signup, login_status
        ...
        def reset_user_data():
            global otp_, trys_, otp_gen_time, email_
            otp_=0
            trys_=0
            otp_hen_time=0
            email_=""
        ...
        def send_mail(i,email,messagee,subject):
            def threaded():
                ...
                try:
                    ...
                    if i=="resend_otp":
                        otp_login_singup.label="OTP"
                        otp_login_singup.hint_text=""
                        otp_login_singup.read_only=False
                    else:
                        otp_login_singup.read_only=False

                    if i=="resend_otp":
                        page_banner=ft.Banner(content=ft.Text("New OTP Sent!"),actions=[ft.TextButton("Dismiss", on_click=close_banner),],bgcolor=ft.colors.GREEN_200,leading=ft.Icon(ft.icons.GPP_GOOD, size=40))
                        page.banner=page_banner
                        page_banner.open=True
                    else:
                        page_banner=ft.Banner(content=ft.Text("OTP sent!"),actions=[ft.TextButton("Dismiss", on_click=close_banner),],bgcolor=ft.colors.GREEN_200,leading=ft.Icon(ft.icons.GPP_GOOD, size=40))
                        page.banner=page_banner
                        page_banner.open=True
                    ...
                    if i=="resend_otp":
                        ...
                        row_remember_load.controls.pop()
                    page_banner=ft.Banner(content=ft.Text("Please check your Email!"),actions=[ft.TextButton("Dismiss", on_click=close_banner),],bgcolor=ft.colors.AMBER_100,leading=ft.Icon(ft.icons.WARNING_AMBER_ROUNDED, color=ft.colors.AMBER, size=40))
                    page.banner=page_banner
                    page_banner.open=True
                page.update()
        ...
        def send_otp(i):
            ...
            if i=="resend_otp" or validate_email.validate_email(email_login_signup.value) or i=="continue_user":
                email_login_signup.read_only=True
                global otp_, trys_, otp_gen_time, email_
                if i=="continue_user":
                    pass
                else:
                    otp_=random.randrange(100000,999999)
                    trys_=0
                    otp_gen_time=datetime.now(timezone("Asia/Kolkata")).strftime("%Y-%m-%d %H:%M:%S:%f")
                if i=="resend_otp":
                    pass
                else:
                    if i=="continue_user":
                        ...
                        email_=email_login_signup.value
                if i=="resend_otp":
                    send_mail("resend_otp",email_, f"{otp_} is OTP to Login/Singup to PlugandPlay don't share this OTP with anyone we never call or ask for OTP.", "PlugandPlay OTP verification message")
                    ...
                    if i=="continue_user":
                        email_login_signup.read_only=False
                        email_login_signup.value=email_
                        email_login_signup.read_only=True
                        otp_login_singup.read_only=False
                        check_otp("continue_user")
                        page.update()
                    else:
                        send_mail("pass",email_, f"{otp_} is OTP to Login/Singup to PlugandPlay don't share this OTP with anyone we never call or ask for OTP.", "PlugandPlay OTP verification message")
            
            else:
                email_login_signup.read_only=False
                page_banner=ft.Banner(content=ft.Text("Invalid Email Address"),actions=[ft.TextButton("Retry", on_click=close_banner),],bgcolor=ft.colors.AMBER_100,leading=ft.Icon(ft.icons.WARNING_AMBER_ROUNDED, color=ft.colors.AMBER, size=40))
                page.banner=page_banner
                page_banner.open=True
        ...
        if login_status==None:
            ...
            bookings_user_login(login_status)
        
        email_login_signup=ft.TextField(expand=True,height=53,suffix_icon=ft.icons.EMAIL_OUTLINED,label="Email")
        otp_login_singup=ft.TextField(expand=True,height=53,password=True, can_reveal_password=True, label="OTP", read_only=True, on_change=lambda i:[check_otp(otp_login_singup.value)])
        ...
        body_login_signup=ft.Column(controls=[ft.Text(),
                                ft.Container(content=ft.Column(controls=[ft.Row(controls=[ft.Text("Login / Sign Up", size=22, color="#062A70", text_align="center", weight=ft.FontWeight.W_900)], alignment="center"),
                                                                        ft.Row(controls=[email_login_signup]),
                                                                        ft.Row(controls=[otp_login_singup]),
                                                                        row_remember_load,
        ...
        if otp_==0: # therefore always set value of otp_=0 after user logs in successfully
            pass
        else:
